...
def get_nasa_epic_photo(count, folder):
    load_dotenv()
...
if __name__ == '__main__':
    command_arguments = argparse.ArgumentParser\
        (description='Загрузка картинок NASA EPIC (Earth Polychromatic Imaging Camera). Скрипт загружает фотографии с '
                     ...
                     'необязательный параметр count. Так же можно указать папку для сохранения картинок, '
                     'по умолчанию - images')
    ...
    try:
        get_nasa_epic_photo(args.count, args.folder)
